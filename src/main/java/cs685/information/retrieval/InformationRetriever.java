...
public class InformationRetriever {
	 ...
	 */
	public InformationRetriever(File root, List<Diff> diffs, String projectName) throws IOException, InterruptedException {
		...
		Map<Diff, File> diffToFilePath = new HashMap<Diff, File>();
		System.out.println("Creating diffToFilePath map!");
		Stack<File> filesToProcess = new Stack<>();
		filesToProcess.push(root);
		while (!filesToProcess.isEmpty()) {
			File currPath = filesToProcess.pop();
			if (currPath.isDirectory()) {
				filesToProcess.addAll(Arrays.asList(currPath.listFiles()));
			} else if (currPath.getName().endsWith(".java")) {
				// Check if file is in our filenameToDiff map
				String currFile = currPath.getAbsolutePath();
				System.out.println("Found Java file: [" + currPath.getName() + "]");
				System.out.println("getAbsolutePath(): [" + currPath.getAbsolutePath() + "]");
				String currFilePathSplit[] = currPath.getAbsolutePath().split(projectName);
				...
				if (currFilePathSplit.length != 2) {
					...
					// Windows fix
					currFilePath = currFilePath.replaceAll("\\\\", "/");
				}
		...
		for (Diff diff : diffToFilePath.keySet()) {
			...
			CompilationUnit cu = JavaParser.parse(diffToFilePath.get(diff));
